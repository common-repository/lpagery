jQuery(function ($) {
    ...
    let last_index = null;
    let post_update_cache_container = new Map();

    let image_data_cache_container = new Map();
    let image_data_header_cache_container = new Map();
    let parent_post_cache_container = new Map();
    let template_post_cache_container = new Map();
    let author_cache_container = new Map();
    let taxonomy_cache_container = new Map();
    let request_cache_container = new Map();

    let sync_manually = false;
    let ram_protection_ignored = false;
    ...
    $.clearModalData = function () {
        ...
        template_post_cache_container = new Map();
        author_cache_container = new Map();
        taxonomy_cache_container = new Map();
        request_cache_container = new Map();
        sync_manually = false;
    ...
    async function start_creation() {
        ...
        let page_creation_id = (Math.random() + 1).toString(36).substring(2);
        ...
        for (let index in data) {
            ...
            $('#lpagery_pause').hide()
            let result = await createPage(element, index, process_id, null, length, page_creation_id);
        ...
        jQuery.ajax({
            ...
            "success": function (response) {
                ...
                finished = true;
                let suffix = lpagery_ajax_object_modal.wpml_installed ? '&lang=all' : '';

                let url = 'edit.php?post_type=' + parsed.type + '&lpagery_process=' + process_id + '&orderby=modified&order=desc' + suffix
    ...
    function getMode() {
        if(jQuery(".lpagery-create-update-tabs").length === 0){
            return 'create';
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            jQuery.ajax({
                ...
                success: function (response) {
                    ...
                    if (parse.process_id <= 0) {
                        onError("Failed to initialize the Syncing Process. This might be due to an incompatible/old database (mysql) version. Please contact your hosting provider to update the database version");
                        return reject();
                    }
    ...
    async function checkNotPassed() {
        ...
        let duplicatedSlugs = []
        if (data.length <= 500 && !sync_manually) {
            var ajaxData = {
                action: 'lpagery_get_post_title',
                post_id: $('#lpagery_template_path').val(),
                "_ajax_nonce": lpagery_ajax_object_root.nonce
            };
            let post_title = await jQuery.get(
                lpagery_ajax_object_root.ajax_url,
                ajaxData
            );
            duplicatedSlugs = await lpagery_get_duplicated_slugs(slugInput.val(), post_title, fields, data);
            if(duplicatedSlugs.exception){
                onError(duplicatedSlugs.exception);
                return true;
            }
        }
        ...
        const addConfirmationMessage = (value, slugs) => {
            ...
            const slugsHTML = slugsToShow.map(slug => {
                if (typeof slug === 'string') {
                    return `<li>${slug}</li>`;
                } else if (slug instanceof jQuery) {
                    // Create a new li element and append the jQuery element to it
                    const li = $('<li></li>').append(slug);
                    return li.prop('outerHTML'); // Get the HTML string of the li element
                }
                return '';
            }).join('');
        ...
        if (duplicatedSlugs.title_contains_placeholder === false) {
            addConfirmationMessage("It seems like <strong>no {placeholder}</strong> from the input sheet is included in the <strong>title of the template page.</strong> Please add at least one placeholder to generate pages with different titles", []);
        } else if (duplicatedSlugs.all_slugs_are_the_same) {
            addConfirmationMessage("It seems like <strong>no {placeholder}</strong> from the input sheet is included in the <strong>configured slug.</strong> Please add at least one placeholder to generate pages with different slugs", [])
        } else {
            if (duplicatedSlugs.filename_slug_equals && duplicatedSlugs.filename_slug_equals.length > 0) {
                let mapped_slugs = duplicatedSlugs.filename_slug_equals.map(duplicate => {
                    return duplicate.value + " at rows " + duplicate.rows.join(", ");
                });
                addConfirmationMessage("<strong>Filenames matching the slug</strong> have been detected in the <strong>current file.</strong> WordPress will add a number to the slugs of the created pages (e.g., 'city' becomes 'city-2'). Use a different filename to avoid this. ", mapped_slugs);
            }

            if (duplicatedSlugs.duplicates && duplicatedSlugs.duplicates.length > 0) {
                let mapped_slugs = duplicatedSlugs.duplicates.map(duplicate => {
                    return duplicate.value + " at rows " + duplicate.rows.join(", ");
                });
                addConfirmationMessage("<strong>Duplicate slugs</strong> detected in the <strong>current file.</strong> Make sure each slug is different. If there are duplicates, they won't be considered.", mapped_slugs);
            }

            if (duplicatedSlugs.numeric_slugs && duplicatedSlugs.numeric_slugs.length > 0) {
                let mapped_numeric_slugs = duplicatedSlugs.numeric_slugs.map(duplicate => {
                    return duplicate.value + " at row " + duplicate.row;
                });
                addConfirmationMessage("<strong>Numeric slugs</strong> detected in <strong>the current file.</strong> Wordpress will automatically attach a number to the slug. For Example '1234' gets '1234-2'", mapped_numeric_slugs);
            }

            if (duplicatedSlugs.existing_slugs && duplicatedSlugs.existing_slugs.length > 0) {
                const slugs = duplicatedSlugs.existing_slugs.map(slug => {
                    return $('<a></a>')
                        .attr('href', slug.permalink)
                        .attr('target', '_blank')
                        .text(slug.post_name);
                });
                addConfirmationMessage("<strong>Duplicate slugs</strong> detected in the <strong>WordPress instance.</strong> Continuing will cause WordPress to add numbers to the URLs. We recommend removing the existing pages or changing the slugs of the new pages", slugs);
            }
        ...
        if (missingFilenames.size > 0) {
            addConfirmationMessage("<strong>Images can not be downloaded</strong> </br>The following images need to have an LPagery Download Filename with placeholders configured. Otherwise, they won't be downloaded. Please go ahead to the Media Library and add the names for the files to be downloaded", Array.from(missingFilenames));
        } else {
        ...
        if (confirmationMessages.length > 0) {
            ...
            allMessages = allMessages + "Do you want to proceed?"
    ...
    function prepare_data_element(element, keys_to_remove_add) {
        ...
        for (const [key, value] of Object.entries(element)) {
            ...
            filtered_value = encodeURI(filtered_value).trim();
        ...
        Object.keys(filtered_element).forEach(key => {
            if (keys_to_remove.includes(key.replaceAll(".", "\u2024"))) {
    ...
    function prepare_grid_data(fields, element, ...keys_to_remove_add) {
        fields.forEach(field => {
            if (element[field.name] == null || element[field.name] === undefined) {
                element[field.name] = ""
            }
        })
    ...
    function prepare_grid_data_for_duplicated_slugs(slug, post_title, element) {
        let keys_to_remove_add = [];
        for (const [key, value] of Object.entries(element)) {
            let lower_key = key?.toLowerCase();
            let lower_slug = slug.toLowerCase();
            let lower_post_title = post_title.toLowerCase();
            if (!lower_slug.includes(lower_key) && !lower_post_title.includes(lower_key) && lower_key !== "lpagery_ignore" && !isImageColumn(key)) {
                keys_to_remove_add.push(key)
            }
        }
    ...
    function prepare_element_trim(element, keys_to_remove_add) {
        let prepareDataElement = prepare_data_element(element, keys_to_remove_add);
        for (const [key, value] of Object.entries(prepareDataElement)) {
            if ((typeof value === 'string' || value instanceof String) && value.length > 500) {
                prepareDataElement[key] = value.substring(0, 50)
            } else {
                prepareDataElement[key] = value;
            }
    ...
    function prepare_element_for_images(element) {
        let keys_to_remove_add = [];
        ...
        for (const [key, value] of Object.entries(element)) {
            if (typeof value !== 'string' || isImageColumn(key)) {
                continue
            }
            if (!imageValues.find(imageValue => imageValue.toLowerCase().includes(key.toLowerCase()))) {
                keys_to_remove_add.push(key)
            }
        }
        return prepare_element_trim(element, keys_to_remove_add);
    }

    function createPage(element, index, process_id, retry_count, total_count, page_creation_id) {
        if (canceled) {
    ...
    function handleCacheAndFetch(headerValue, value, item, fetchFunction, cacheContainer, setHtmlFunction) {
        let container = $("<div>");
        if (!cacheContainer.has(headerValue)) {
            cacheContainer.set(headerValue, new Map());
        }
        let cacheKey = headerValue + item?.lpagery_row_id;
        let cache = cacheContainer.get(headerValue);
        let cached = cache.has(item?.lpagery_row_id);

        if (cached) {
            setHtmlFunction(cache.get(cacheKey), container);
        } else {
            container.append(value);
            if (!request_cache_container.has(cacheKey)) {

                // Create and store the promise
                let fetchPromise = fetchFunction(container, item, value, headerValue);
                request_cache_container.set(cacheKey, fetchPromise);
            }

            // Use the existing promise to update the UI once the request completes
            request_cache_container.get(cacheKey).then(parsed => {
                cache.set(cacheKey, parsed.data);
                setHtmlFunction(parsed.data, container);
            }).catch(error => {
                console.error(error);
                container.append(trimForGrid(value));
            });
        }
        return container;
    }
    ...
    $.lpageryGetHeaders = function getHeaders(headers) {
        return new Promise(async (resolve, reject) => {
                ...
                if (allowedPlaceholders) {
                    if (!!allowedPlaceholders.placeholders) {
                        ignore_fields = allowedPlaceholders.placeholders
                        if (headers.slice(ignore_fields)) {
                ...
                if(image_column_count > 0 &&  !lpagery_ajax_object_modal.is_extended_plan){
                    Toastify({
                        text: 'It looks like you\'d like to use image processing. To get started, please upgrade to the extended plan.',
                        duration: 6000,
                        close: true,
                ...
                else if (image_column_count > 0 && !image_processing_enabled) {
                    Toastify({
                ...
                if ((image_column_count > 5) && image_processing_enabled && lpagery_ajax_object_modal.is_extended_plan) {
                    Toastify({
                ...
                let mapped_headers = headers.map(headerValue => {
                    if (!lpagery_ajax_object_modal.is_free_plan && lpagery_ajax_object_modal.is_extended_plan) {
                        if (isImageColumn(headerValue) && image_column_count <= 5) {
                            ...
                            return {
                                "name": "lpagery_parent", "title": "Parent", itemTemplate: function (value, item, test) {
                                    ...
                                    if (parent_post_cached) {
                                        ...
                                        fetch_and_set_post(existing_post_container, item, value, "parent", parent_post_cache_container);
                                    }
                        ...
                        if (headerValue === "lpagery_template") {

                            return {
                                "name": "lpagery_template",
                                "title": "Template",
                                itemTemplate: function (value, item, test) {

                                    var existing_post_container = $("<div>");
                                    let template_post_cached = template_post_cache_container.has(item?.lpagery_row_id);
                                    if (template_post_cached) {
                                        set_page_html(template_post_cache_container.get(item?.lpagery_row_id), existing_post_container);
                                    } else {
                                        existing_post_container.append(trimForGrid(value));
                                        fetch_and_set_post(existing_post_container, item, value, "template", template_post_cache_container);
                                    }
                        ...
                        if (headerValue === "lpagery_categories") {
                            return {
                                ...
                                itemTemplate: function (value, item) {
                                    return handleCacheAndFetch(headerValue, value, item, fetch_and_set_taxonomies, taxonomy_cache_container, set_taxonomies_html);
                                }
                            }
                        }


                        if (headerValue.startsWith("lpagery_taxonomy_")) {
                            return {
                                "name": headerValue,
                                headerTemplate: function () {
                                    const $container = $("<div>");
                                    let ajaxData = {
                                        "action": 'lpagery_get_taxonomy_name',
                                        "taxonomy": headerValue,
                                        "_ajax_nonce": lpagery_ajax_object_modal.nonce
                                    };
                                    ...
                                    jQuery.ajax({
                                        ...
                                        "success": function (response) {
                                            let parsed = JSON.parse(response);
                                            if (!parsed.taxonomy_name) {
                                                $container.append(headerValue + ' not found')
                                            } else
                                                $container.append(parsed.taxonomy_name + ' (Taxonomy)')
                                        }
                                    });
                                    return $container
                                },
                    ...
                    if (ignore_fields && count >= ignore_fields) {
                        return {
                            ...
                            headerTemplate: function () {
                                ...
                                $('<p>' + headerValue + '</p>')
                                return $('<span>' + headerValue + '</span><img class="img-pro" src="' + pro_img + '" alt="image" width="30" height="30" title="No image with name ' + headerValue + ' found">')
        ...
        return new Promise((resolve, reject) => {
            jQuery.ajax({
                ...
                "success": function (response) {
                    ...
                    resolve({data: page_to_be_updated.post});
                },
                "error": function (response) {
                    reject(response);
                }
    ...
    function fetch_and_set_post(existing_post_container, item, value, type, cache_container) {
        item = prepare_element_trim(item);
        let ajaxData = {
        ...
        jQuery.ajax({
            ...
            "success": function (response) {
                let parsed = JSON.parse(response);
                cache_container.set(Number(item.lpagery_row_id), parsed.post)
    ...
    function fetch_and_set_taxonomies(category_container, item, value, fieldName) {
        item = prepare_element_trim(item);

        let ajaxData = {
            "action": 'lpagery_get_hierarchical_taxonomies',
            "data": JSON.stringify(item),
        ...
        return new Promise((resolve, reject) => {
            jQuery.ajax({
                ...
                "success": function (response) {
                    let parsed = JSON.parse(response);
                    resolve({data: parsed.hierarchical_taxonomies});
                },
                "error": function (response) {
                    reject(response);
                }
    ...
    function load_grid(data, headersFields) {
        ...
        if (lpagery_existing_post_index < 0 && (getMode() === "update" || sync_manually)) {
            let existing_post_column = {
                ...
                itemTemplate: function (value, item, test) {
                    ...
                    return handleCacheAndFetch("lpagery_existing_post", value, item, fetch_and_set_page_to_be_updated, post_update_cache_container, set_page_html);
                }
            }
    ...
    $.lpageryShowModal = async function showModal(headersFields, data, sync_manually_param, process_id_param) {
        ...
        if (getMode() =='update' || sync_manually) {
            update_mode = true
    ...
    function onError(...errors) {
        ...
        canceled = true;
        let errorsString = '\n' + errors.map(s => '\n' + s).toString();
        $('#lpagery_error-content').html('<br><b>' + errorsString + "</b>");
    }
    ...
    function isImageColumn(name) {
        ...
        let replaced_name = name.replaceAll("\u2024", ".")
        return ['.png', '.jpg', '.jpeg', '.heic', '.gif', '.svg', '.webp'].some(ending => replaced_name.endsWith(ending))
    }
    ...
    async function lpagery_get_duplicated_slugs(slug, post_title, headers, data) {
        return new Promise((resolve, reject) => {
            let prepared_data = data.map(element => prepare_grid_data_for_duplicated_slugs(slug, post_title, element));
    ...
    function set_taxonomies_html(taxonomies, taxonomy_container) {
        taxonomy_container.empty()
        if (!taxonomies) {
            taxonomy_container.append("-");
        } else {
            const $ul = $('<ul>');
            $.each(taxonomies, function (index, taxonomy) {
                if (taxonomy && taxonomy.taxonomies) {
                    var categoryText = taxonomy.taxonomies.join(' > ') + (!taxonomy.exists ? ' (+)' : '');
                    var $li = $('<li>').text(categoryText);
                    $ul.append($li);
                }
            });
            taxonomy_container.append($ul);
        }
    }
    ...
    function get_and_set_image_cell_value(header_value, data, image_container, value) {
        ...
        data = prepare_element_for_images(data)
        var ajaxData = {
            "action": 'lpagery_get_image_urls',
            "header_name": encodeURIComponent(header_value),

            ...
            fields: [{
...

                {
                    ...
                    name: "post_count", title: "#", width: 10, itemTemplate: function (value, item) {
                        let suffix = lpagery_ajax_object_history.wpml_installed ? '&lang=all' : '';

                        return $("<a>").attr("href", 'edit.php?post_type=' + item.post_type + '&lpagery_process=' + item.id + '&orderby=date&order=desc' + suffix).text(value);
                    ...
                    itemTemplate: function (value, item) {
                            ...
                            $menuContainer.append($("<div>").addClass("lpagery-menu-item").html("Google Sheet Sync Settings").on("click", function (e) {
                                ...
                                jQuery.ajax({
                                    ...
                                    "success": function (response) {
                                        ...
                                        if(parsed.count_data && parsed.status ==='RUNNING') {
                                            $('#lpagery_google_sheet_status').html(get_pretty_status(parsed.status) + " (" + parsed.count_data.current + " of " +  parsed.count_data.total +  " Finished)");
                                        }
                        ...
                        $menuContainer.append($("<div>").addClass("lpagery-menu-item").html("Download Data").on("click", function (e) {
                            ...
                            jQuery.get(lpagery_ajax_object_history.ajax_url, ajaxData, function(response) {
                                ...
                                XLSX.utils.book_append_sheet(wb, ws, "LPagery Export");

                                // Convert workbook to binary XLSX
                                let wbout = XLSX.write(wb, {bookType:'xlsx', type:'array'});

                                // Save as XLSX file
                                let file_name = "lpagery_export_" + parsed.process.raw_purpose + "_" + parsed.process.created + ".xlsx";
                                file_name = file_name.toLowerCase().replaceAll(" ", "_");
                        ...
                        $menuContainer.append($("<div>").addClass("lpagery-menu-item").html("Export Template Page").on("click", function (e) {
                            e.stopPropagation();
                            $menuContainer.hide();
                            var ajaxData = {
                                action: 'lpagery_download_post_json',
                                "_ajax_nonce": lpagery_ajax_object_root.nonce,
                                "process_id" : item.id
                            };
                            window.open(lpagery_ajax_object_history.ajax_url + "?" + jQuery.param(ajaxData));
